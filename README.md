...
| 操作员 | `user` | `user123` | 仅点检与查看 |

> ⚠️ 上线前务必到「管理后台 → 修改密码」或「用户管理」改掉默认口令。
...
- 仓库已附带一份**已脱敏演示** `data/config.json`（演示密钥）与 `data/kv.json`：设备编号、车间名（车间A / 车间B）、型号（设备型号-X）、签名人（演示签核人）均为虚构占位，**不含任何真实人员姓名、真实设备序列号、真实手写签名或企业内部信息**。克隆即可体验；上线前请替换为你的真实数据并重新生成密钥（删除 `data/config.json` 重启即重置）。
...
去重后整库 `STORE` 约 **0.9 MB**，远低于 25 MB 上限，无需引入 R2 对象存储。（若未来数据远超 25 MB，可改走 R2 分片存储。）
